...
geospatial <- read_csv(here("Output_for_analysis", "06_Meta_geospatial_extraction_with_comids", "Geospatial_data_2023-11-13.csv"))
...
study_site_data <- read_excel(here("inputs", "catchment_characteristics", "Fire_name_Lat_Long.xlsx")) %>% 
...
write_csv(effect_size_fire, here("Output_for_analysis", "07_meta_effect_size_models", "effect_size_geospatial_fire_lat_long.csv"))
...
### ALANs comments of no gridlines/every other burn percentage ####
ggplot(effect_both_doc_no3, aes(burn_percentage_group, Effect_size, color = response_var),
       position = position_dodge(width = -0.5)) +
  geom_jitter(position = position_jitter(0.1), alpha = 0.4, size = 3) +
  geom_pointrange(aes(ymin = lower_ci_effect, ymax = upper_ci_effect,
  ...
  stat_summary(fun.data = give.n, geom = "text", fun.y = median,
                  position = position_nudge(x = -0.4)) +
  scale_x_discrete(expand = c(0.1, 0.1), limits = c("0-5", "6-10", "11-15", "16-20", "21-25", "26-30", "31-35", "36-40","41-45", "46-50", "51-55", "56-60", "61-65", "66-70", "71-75", "76-80", "81-85", "86-90","91-95", "96-100")) +
  ylim(-5, 5) +
  theme_classic() +
  theme(axis.text.x = element_text(size = 30, angle = -90),
        axis.text.y = element_text(size = 30),
...
ggsave("Effect_Size_Burn_Percentage_everyothe.pdf",
       path = ("~/GitHub/rc_sfa-rc-3-wenas-meta/FireMeta_Rproj/initial_plots/03_Wenas_effect_size"),
       width = 10, height = 8, units = "in")
...
ggsave("Effect_Size_Burn_Percentage_10s.pdf",
       path = ("~/GitHub/rc_sfa-rc-3-wenas-meta/FireMeta_Rproj/initial_plots/03_Wenas_effect_size"),
       width = 10, height = 8, units = "in")
...
# Test for grouping 
df <- effect_both_doc_no3 %>%
  mutate(grouped_percentage = cut(burn_percentage, breaks = seq(0, 100, by = 20), include.lowest = TRUE, labels = FALSE))

df <- df %>% 
  mutate(grouped_percentage = as.character(grouped_percentage))

effect_both_doc_no3 <- df %>% 
  mutate(burn_percentage_group = case_when(grouped_percentage == "1" ~ "0-20",
                                           grouped_percentage == "2" ~ "21-40",
                                           grouped_percentage == "3" ~ "41-60",
                                           grouped_percentage == "4" ~ "61-80",
                                           grouped_percentage == "5" ~ "81-100"))
                                           

#

effect_both_doc_no3$burn_percentage_group <- factor(effect_both_doc_no3$burn_percentage_group, levels = c("0-20", "21-40", "41-60", "61-80", "81-100"))
...
ggplot(effect_both_doc_no3, aes(burn_percentage_group, Effect_size, color = response_var),
  ...
  stat_summary(fun.data = give.n, geom = "text", fun.y = median,
                  position = position_nudge(x = -0.4)) +
  scale_x_discrete(expand = c(0.1, 0.1), limits = c("0-20", "21-40", "41-60", "61-80", "81-100")) +
  ylim(-5, 5) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 30, angle = -90),
        axis.text.y = element_text(size = 30),
...
ggsave("Effect_Size_Burn_Percentage_20s.pdf",
       path = ("~/GitHub/rc_sfa-rc-3-wenas-meta/FireMeta_Rproj/initial_plots/03_Wenas_effect_size"),
       width = 10, height = 8, units = "in")
