...
no3 <- read_csv(here("Output_for_analysis", "08_Meta_summary_stats", "no3_meta_random_forest.csv")) %>% 
  ...
  select(-c(minelevraw, elevfixed, minelevsmo))
...
# The above links are yielding different plots then what Peter had originally given me. Lets test out the original files that I sent Peter to see if they are different. 
no3.peter <- read_csv("~/Desktop/R_tutorials/no3_meta_random_forest_toPeter.csv") %>% 
  clean_names() %>% 
  rename("dependent" = effect_size) %>% 
  select(-c(minelevraw, elevfixed, minelevsmo))

doc.peter <- read_csv("~/Desktop/R_tutorials/doc_meta_random_forest_toPeter.csv") %>% 
  clean_names() %>% 
  rename("dependent" = effect_size) %>% 
  select(-c(minelevraw, elevfixed, minelevsmo))

merged_no3 <- merge(no3.peter, no3, by = "dependent", all = TRUE)

# Check if values match
values_match <- merged_no3$dependent == merged_no3


```

```{r chat gpt merging test}
# Sample dataframes
df1 <- data.frame(ID = c(1, 2, 3), Value1 = c("A", "B", "C"))
df2 <- data.frame(ID = c(1, 3, 4), Value2 = c("X", "Y", "Z"))

# Merge dataframes based on the ID column
merged_df <- merge(df1, df2, by = "ID", all = TRUE)

# Check if values match
values_match <- merged_df$Value1 == merged_df$Value2
