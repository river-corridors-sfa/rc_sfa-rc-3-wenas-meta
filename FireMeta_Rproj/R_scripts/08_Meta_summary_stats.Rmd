...
```{r}
geospatial_data<- read_csv(here("Output_for_analysis", "06_Meta_geospatial_extraction_with_comids", "Geospatial_data_2024-01-18.csv"))
names(geospatial_data)
# It doesnt look like they have any vegetation data. 
#They have size of watershed, elevation, slope 

q <- geospatial_data %>% 
  select(site, qa_ma, qc_ma)
```
...
doc <- final_dataset %>% 
  ...
  dplyr::select(Effect_size, Climate, totdasqkm, minelevraw, maxelevsmo, minelevsmo, elevfixed, slope, burn_percentage, Time_Since_Fire) %>% # Only want to do the catchment characteristics
  na.omit()
...
library(tidyverse)
library(hrbrthemes)
watershed_area <- read_csv("~/OneDrive - PNNL/Documents/RC_3/Wenas_Watershed_project/Manuscript/Watershed_area.csv") 
