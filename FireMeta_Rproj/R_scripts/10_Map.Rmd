...
```{r - Chat GPT trying to make AK an inset map}
...
us_map + inset_element(alaska_map, 0, -0.8, 0.4, 1, align_to = 'full')

# Now lets figure out how to get the points on the map and create an insert that way #

# continuous US with points 
us <- ggplot() +
  geom_sf(data = us_states, fill = "white", color = "black") +
  geom_sf(data = coordinates_sf, aes(size = number_of_burn, color = Fire_Name)) +
  scale_size_continuous(guide = guide_legend(title = "Number of Watersheds")) +
  scale_color_manual(values = c("#666666", "#A6CEE3", "#00AFBB", "#1B9E77",
                                "#7570B3", "#E7298A", "darkred", "black", 
                                "#1F78B4", "#E31A1C", "#33A02C", "#FB9A99",
...
# merge the two maps with points as an insert
us + inset_element(ak, 0, -1.0, 0.4, 1, align_to = 'full')
...
bc <- world[world$name == "Canada" & world$admin == "British Columbia", ]

# take 2 #
bc_shapefile <- st_read("path_to_shapefile/canada_provinces.shp")

ggplot() +
  geom_sf(data = world, fill = "lightgray", color = "white", size = 0.2) +
  geom_sf(data = bc, fill = "blue", color = "white", size = 0.8) +
  theme_minimal()
...
us_ca + inset_element(ak, 0, -1.0, 0.4, 1, align_to = 'full')
