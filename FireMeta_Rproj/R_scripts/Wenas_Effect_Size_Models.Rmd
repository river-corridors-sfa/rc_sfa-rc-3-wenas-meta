...
effect_size <- read_csv(here("Output_for_analysis", "Effect_Size.csv")) %>% 
  select(Study_ID, response_var, Climate, Biome, Effect_size, Site, Time_Since_Fire)
...
study_site_data <- read_excel(here("inputs", "Studies_Summary", "Fire_name_Lat_Long.xlsx")) %>% 
  select(Study, Site, Fire_year, Effect_size_site) %>% 
  rename(site = Site, 
         Study_ID = Study, 
         year = Fire_year, 
