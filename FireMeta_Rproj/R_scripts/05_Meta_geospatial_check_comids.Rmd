...
sites <- read_csv(here("inputs", "catchment_characteristics", "watershed_lat_long.csv")) %>%
...
updated_sites <- tibble(site = c("Site 3", "Reference", "Payson", "Dry Creek"),comid = c(8943613, 15032969, 10350360, 10329181)) 
...
write.csv(sites, here("Output_for_analysis", "05_Meta_geospatial_check_comids", "geospatial_data.csv"))


write.csv(site_watersheds, here("Output_for_analysis", "05_Meta_geospatial_check_comids", "meta_comids.csv"))
...
write.csv(sites_export, here("inputs", "catchment_characteristics", "meta_lat_long_comids.csv"))
...
#### Fire Permiter behind the watershed Figure for AGU_2023 ####
```{r}

# Only want the Gaviota sites 
sites <- read_csv(here("inputs", "Studies_Summary", "Coombs_Melack_Coords.csv")) %>%
  distinct(site, .keep_all = TRUE) %>%
  dplyr::select('site','latitude','longitude') %>% 
  drop_na()
...
m1 <- mapview(st_read(here("gis_data", "Fire_Perimeters", "Gaviota", "ca3448712019620040605_20040425_20050412_burn_bndy.shp")), lwd = 3, col.regions = "red", color = "red", quiet = T, alpha.regions = 0.4, layer.name = "Fire Perimeter") +
  mapview(site_watersheds, col.regions = "#56B4E9", alpha.regions = 0.2, lwd = 3, layer.name = "Watershed") +
  mapview(sites, cex = 5, col.regions = "black", layer.name = "Points")

mapshot(m1, file = "~/GitHub/rc_sfa-rc-3-wenas-meta/FireMeta_Rproj/initial_plots/Maps/gaviota.pdf", format = "pdf")
