...
coords <- meta %>% 
  select(Fire_Name, Lat, Long, number_of_pairs, State, group)

# attempt to make sites bigger size with more sites


# Load your CSV file with coordinates and conditional variable
# Replace "your_data.csv" with the actual file path and name
data <- coords %>% 
    drop_na(Fire_Name)

# Assuming your CSV file has columns "longitude", "latitude", and "conditional_variable"
...
ggplot() +
  ...
  geom_sf(data = coordinates, aes(size = number_of_sites)) +
...
ggplot() +
  geom_sf(data = us_states, fill = "white", color = "black") +
  geom_sf(data = coordinates, aes(size = number_of_pairs, color = Fire_Name)) +
  scale_size_continuous(guide = guide_legend(title = "Number of Paired Watersheds")) +
  scale_color_manual(values = c("#666666", "#00AFBB", "#666666", "#1B9E77", "#1B9E77", "#7570B3", "#E7298A", "darkred", "blue", "red", "black", "#A6CEE3", "#1F78B4", "#E31A1C", "#33A02C", "#FB9A99", "#1B9E77"), 
