...
site_data <- control_impact_units %>% 
  select(Study_ID, Latitude, Longitude, Area_watershed_km, Climate, Burn_Unburn,
         Time_Since_Fire)
```
...
monthly_mean_site <-  metadata_long %>%
  ...
  left_join(site_data, by = "Study_ID") %>% 
  group_by(month) %>% 
...
ggplot(monthly_mean_site, aes(x = log(TempNorm), y = Climate, color = response_var),
