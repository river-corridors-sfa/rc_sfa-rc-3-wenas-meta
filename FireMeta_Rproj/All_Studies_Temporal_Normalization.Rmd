...
site_data <- control_impact_units %>% 
  select(Study_ID, Pair, Latitude, Longitude, Area_watershed_km, Climate, Burn_Unburn, Time_Since_Fire)
...
range <- control_impact_fill %>% 
  group_by(Study_ID) %>% 
  summarise(Range = max(Sampling_Date) - min(Sampling_Date))
...
control_impact_fill$Time_Since_Fire <- factor(control_impact_fill$Time_Since_Fire, levels = c("0", "1", "2", "3", "4", "5", "50"))
...
yearly_effect_size_combine <- yearly_effect_size_combine %>% 
  mutate(yearly_effect_size_combine, Biome = ifelse(grepl("Dfb|Dfc|Dsb|Dwc", Climate), "Cold",
...
#   filter(Study_ID == "Crandall et al. 2021")
# 
# write.csv(crandall, here("Output_for_analysis", "aggregated_effect_crandall_4sites.csv"))
...
# geom_jitter for TSF # 
ggplot(yearly_effect_size_combine_long, aes(Effect_size, Time_Since_Fire, color = response_var),
       position = position_dodge(width = -0.5)) +
  geom_jitter(position = position_jitter(0.1), alpha = 0.4, size = 3) +
  geom_pointrange(aes(xmin = lower_ci_effect, xmax = upper_ci_effect,
                      color = response_var),
...
ggsave("Effect_Size.TSF_Vertical.pdf",
       path = ("~/GitHub/rc_sfa-rc-3-wenas-meta/FireMeta_Rproj/initial_plots/03_Wenas_effect_size"),
       width = 10, height = 8, units = "in")
...
ggsave("Effect_Size.TSF_Horizontal.pdf",
       path = ("~/GitHub/rc_sfa-rc-3-wenas-meta/FireMeta_Rproj/initial_plots/03_Wenas_effect_size"),
       width = 10, height = 8, units = "in")
