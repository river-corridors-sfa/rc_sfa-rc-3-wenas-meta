...
```{r}
# combine the yearly_effect_size_combine_long with Crandall and above_five_effect_size

Crandall_effect_size <- read_csv(here("Output_for_analysis", "Crandall_effect_size.csv")) %>% 
  select(-...1)

above_five_effect_size <- read_csv(here("Output_for_analysis", "greater_than_5_years_effect_size.csv")) %>% 
  select(-...1)
...
yearly_effect_size_combine_long <-yearly_effect_size_combine_long %>% 
  add_column(Xe6 = NA,
             ...
             Xc34 = NA) %>% 
  ungroup() %>% 
  select(Study_ID, response_var, Climate, year, Xe1, Xe2, Xe3, Xe4, Xe5, Xe6, Xe7, Xe8, Xe9, Xe10,
         Xe11, Xe12, Xe13, Xe14, Xe15, Xe16, Xe17, Xe18, Xe19, Xe20, Xe21, Xe22, Xc, Xc2, Xc34, Xc345, Biome, Site, Effect_size)
...
# Adding columns to above_five_effect_size to match yearly_effect_size_combine_long and Crandall_effect_size 
above_five_effect_size <-above_five_effect_size %>% 
  add_column(Xc2 = NA, 
             Xc34 = NA,
             Xc345 = NA) %>% 
  ungroup() %>% 
...
yearly_effect_size_combine_long <- yearly_effect_size_combine_long %>% 
  ...
  add_row(above_five_effect_size)



### TESTING TO SEE IF THIS PAIRS CORRECTLY ####
crandall <- yearly_effect_size_combine_long %>% 
  filter(Study_ID == "Crandall et al. 2021") # This checks out. Lovely
...
ggplot(yearly_effect_size_combine_long, aes(Effect_size, Climate, color = response_var),
       position = position_dodge(width = -0.5)) +
  ...
  ylab("Koppen (Climate Classification)") +
  xlab("Effect_size") +
  geom_segment(aes(x = 6.25, xend = 6.25, y = 9.5, yend = 5.5), color = "black") +
  geom_segment(aes(x = 5.5, xend = 6.25, y = 9.5, yend = 9.5), color = "black") +
