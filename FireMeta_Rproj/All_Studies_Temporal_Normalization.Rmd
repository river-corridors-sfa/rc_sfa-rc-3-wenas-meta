...
# burd is good
# Coombs & Melack is good
# Gerla & Galloway, 1998 is good
# Gluns & Toews, 1989 is good

manual_effect_size <- manual_effect_size %>% 
  mutate(Time_Since_Fire = case_when(Study_ID == 'Mast & Clow, 2008' & 
                                     year == '2003' ~ "0",
                                     TRUE ~ Time_Since_Fire))
# Mast & Clow, 2008 is good 
# Murphy et al. 2015 is good
...
# Murphy et al. 2018 is good
# Neary & Currier, 1982 is good 
# Oliver et al. 2012 is good 
manual_effect_size <- manual_effect_size[-c(90:91), ] # eliminating bad Rhea data 
# Rhea et al. 2021 is good 
# Tiedemann, 1997 is good 
# Uzun et al. 2020 is good 
# Wagner et al. 2015 is good 
# Writer et al. 2014 is good 
# Hauer & Spencer 1998 is good 
# Hickenbottom et al. 2023 is good 
# Crandall et al. 2021 is good 
# Abbott et al. 2021 is good
...
```{r testing Confidence intervals}
# Confidence Interval just for Cfa
cfa <- yearly_effect_size_combine_long %>% 
  filter(Climate == "Cfa")

cfa_summary_effect <- cfa %>% 
  group_by(response_var, Climate) %>% 
  summarise(mean_effect = mean(Effect_size, na.rm = TRUE),
            sd_effect = sd(Effect_size, na.rm = TRUE),
            n_effect = n(),
...
ggplot(under_ten, aes(Effect_size, Climate, color = response_var),
  ...
  geom_pointrange(aes(xmin = lower_ci_effect, xmax = upper_ci_effect,
                      color = response_var),
                  position = position_dodge(width = -0.5), size = 0.75, data = climate_summary_effect)
...
log(Burn/unburn) # 0.693 
# Positive effect size = burn is higher concentration than the unburn 

burn = 5
unburn = 10
log(burn/unburn) # -0.6931472
# negative effect size = burn is lower concentration than the unburn 
#

percent_difference <- yearly_effect_size_combine_long %>% 
  mutate(pd = (100*(1-exp(Effect_size)))*-1) %>% 
  select(Study_ID, response_var, Site, Climate, Biome, Time_Since_Fire, Effect_size, pd) # creating percent difference 

# caculating confidence intervals # 

TSF_summary_effect <- percent_difference %>%
  group_by(Time_Since_Fire, response_var) %>%
  summarise(mean_effect = mean(pd, na.rm = TRUE),
            sd_effect = sd(pd, na.rm = TRUE),
            n_effect = n(),
...
# TSF 
ggplot(percent_difference, aes(Time_Since_Fire, pd, color = response_var),
       position = position_dodge(width = -0.5)) +
  geom_jitter(position = position_jitter(0.1), alpha = 0.4, size = 3) +
  geom_pointrange(aes(ymin = lower_ci_effect, ymax = upper_ci_effect,
                      color = response_var),
...
percent_difference_test %>% 
  ...
  ggplot(aes(Time_Since_Fire, pd, color = response_var)) +
  geom_point() +
  stat_poly_line() +
...
lm.mod <- lm(pd ~ Time_Since_Fire, data = doc_filter)
summary(lm.mod) # slope = -12.5

model <- theil_sen_regression(pd ~ Time_Since_Fire, data = doc_filter)
summary(model) # slope = -6.991

model_full <- theil_sen_regression(pd ~ Time_Since_Fire, data = doc)
summary(model_full) # slope = -3.748

# plot for supplemental # 
ggplot(doc, aes(Time_Since_Fire, pd, color = response_var)) +
  geom_point() +
  stat_poly_line() +
  stat_poly_eq(use_label(c("eq", "R2"))) +
  geom_hline(yintercept = 0, linewidth = 0.5, color = "red") +
...
ggsave("DOC_Percent_Difference_Scatter.pdf",
       path = here("initial_plots", "Percent_difference"),
       width = 10, height = 8,  units = "in")
...
no3_theil <- theil_sen_regression(pd ~ Time_Since_Fire, data = no3)
summary(no3_theil) # Slope: 26.31

# plot for supplemental # 
ggplot(no3, aes(Time_Since_Fire, pd, color = response_var)) +
  geom_point() +
  stat_poly_line() +
  stat_poly_eq(use_label(c("eq", "R2"))) +
  geom_hline(yintercept = 0, linewidth = 0.5, color = "red") +
