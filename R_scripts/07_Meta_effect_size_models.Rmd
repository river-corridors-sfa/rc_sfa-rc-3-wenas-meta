...
MTBS_model <- full_join(severity_clip, effect_size) %>% 
  dplyr::select(Study_ID, Site, response_var, Climate, Biome, year, Effect_size_pair, Effect_size, Low_Severity, Moderate_Severity, High_Severity, Time_Since_Fire) %>% 
  na.omit()
...
doc_es_mtbs <- MTBS_model_long %>% 
  filter(response_var == "DOC_Interp")

no3_es_mtbs <- MTBS_model_long %>% 
  filter(response_var == "NO3_Interp")
