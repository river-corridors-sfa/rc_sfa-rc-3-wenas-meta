...
geospatial <- read_csv(here("Output_for_analysis", "06_Meta_geospatial_extraction_with_comids", "random_forest_data.csv"))
...
performance::check_heteroscedasticity(doc_bp_mixed) #OK: Error variance appears to be homoscedastic (p = 0.950).
...
# Check for equal variance
performance::check_heteroscedasticity(no3_bp_mixed) #OK:Error variance appears to be homoscedastic (p = 0.957).
...
summary(no3_bp_mixed) # df: 24.348285, t-value: -0.497, p-value: 0.624
fixef(no3_bp_mixed)
...
### Burn percentage continuous model ###
```{r Continuous BP plot}
# Plot #
vn = expression(paste(""*N*O[3]^"-"))
...
# Creating continuous burn percentage plot data frame 
continuous_burn_percentage_plot <- BP_model %>% 
  na.omit()

ggplot(continuous_burn_percentage_plot, aes(burn_percentage, Effect_size, color = response_var),
       position = position_dodge(width = -0.5)) +
  geom_jitter(position = position_jitter(0.1), alpha = 0.4, size = 3) +
...
# Nitrate specific
ggplot(no3_es_bp, aes(burn_percentage, Effect_size, color = response_var),
       position = position_dodge(width = -0.5)) +
  geom_jitter(position = position_jitter(0.1), alpha = 0.4, size = 3) +
  geom_smooth(method = "lm", se = FALSE) +
  stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
...
# Check for equal variance
performance::check_heteroscedasticity(no3_bp_cat) #OK: Error variance appears to be homoscedastic (p = 0.954).
...
#
unique(study_site_data$Study_ID) # Should be 16 sites 

# clip and merge dataframes
severity_clip <- left_join(study_site_data, all_long) %>% 
  dplyr::select(!comid)

unique(severity_clip$Study_ID) # Still 16

severity_clip <- severity_clip %>% 
  na.omit()

unique(severity_clip$Study_ID) # 10 sites are in the analysis 
...
unique(MTBS_model$Study_ID) # 10 studies. Should match the severity_clip data frame
...
### Burn severity models 
```{r read in DNBR severity data frame}
...
# Continuous models with DNBR values #
```{r DOC burn severity models}
doc_sev_mixed <-  lmer(Effect_size ~ mean_dnbr + (1 | Site), data = doc_es_dnbr)

# Check for equal variance
performance::check_heteroscedasticity(doc_sev_mixed) #OK: Error variance appears to be homoscedastic (p = 0.990).

# Check for normality
residuals <- resid(doc_sev_mixed)
qqnorm(residuals)
qqline(residuals)
shapiro.test(residuals) # p-value is < 0.05
# when p > 0.05 can assume normality 
# So technically this is not normally distributed

doc_sev_mixed

summary(doc_sev_mixed) # df: 7.867850, t-value: -2.467, p-value: 0.0394 *
fixef(doc_sev_mixed)
confint(doc_sev_mixed)

predictInterval(doc_sev_mixed)

REsim(doc_sev_mixed) 

plotREsim(REsim(doc_sev_mixed))

# Log transform #
doc_sev_mixed_log <-  lmer(Effect_size ~ log(mean_dnbr) + (1 | Site), data = doc_es_dnbr)

# Check for equal variance
performance::check_heteroscedasticity(doc_sev_mixed_log) #OK: Error variance appears to be homoscedastic (p = 0.992).

# Check for normality
residuals <- resid(doc_sev_mixed_log)
qqnorm(residuals)
qqline(residuals)
shapiro.test(residuals) # p-value is < 0.05
# when p > 0.05 can assume normality 
...
```{r NO3 burn severity models}
no3_sev_mixed <-  lmer(Effect_size ~ mean_dnbr + (1 | Site), data = no3_es_dnbr)

# Check for equal variance
performance::check_heteroscedasticity(no3_sev_mixed) #OK:Error variance appears to be homoscedastic (p = 0.955).

# Check for normality
residuals <- resid(no3_sev_mixed)
qqnorm(residuals)
qqline(residuals)
shapiro.test(residuals) # p-value is > 0.05
# when p > 0.05 can assume normality 
...
summary(no3_sev_mixed) # df: 17.91, t-value: 0.030, p-value: 0.976
fixef(no3_sev_mixed)
confint(no3_sev_mixed)

predictInterval(no3_sev_mixed)

REsim(no3_sev_mixed) 

plotREsim(REsim(no3_sev_mixed))

```

# Categorical models with burn severity categories #
```{r DOC burn severity models}
doc_sev_mixed <-  lmer(Effect_size ~ burn_severity_category + (1 | Site), data = doc_es_dnbr)

# Check for equal variance
performance::check_heteroscedasticity(doc_sev_mixed) #OK: Error variance appears to be homoscedastic (p = 0.927).
...
summary(doc_sev_mixed) # df: 6.0370, t-value: -7.126, p-value: 0.000374 ***
fixef(doc_sev_mixed)
confint(doc_sev_mixed)
...
```{r NO3 burn severity models}
no3_sev_mixed <-  lmer(Effect_size ~ burn_severity_category + (1 | Site), data = no3_es_dnbr)

# Check for equal variance
performance::check_heteroscedasticity(no3_sev_mixed) #OK:Error variance appears to be homoscedastic (p = 0.950).
...
summary(no3_sev_mixed) # df: 16.796, t-value: -1.231, p-value: 0.235
fixef(no3_sev_mixed)
confint(no3_sev_mixed)
...
```{r NO3 levenes test for burn severity}
# Null: the variance amoung the groups is equal. 
# conduct Levene's Test for equality of variances
leveneTest(Effect_size ~ burn_severity_category, data = no3_es_dnbr) #F-value: 1.0535, p-value: 0.3551, Df: 2,60

# plot 

boxplot(Effect_size ~ burn_severity_category,
  data = no3_es_dnbr,
  main = "Effect size Distribution by Burn Severity - NO3",
  xlab = "Burn Severity",
  ylab = "Effect Size",
  col = "#E7B800",
  border = "black")
...
percentage_data <- no3_es_dnbr %>%
  count(burn_severity_category) %>%
  mutate(percentage = n / sum(n) * 100)
```
The p-value of the test is 0.3551, which is greater than our significance level of 0.05. 
Thus, we DO NOT reject the null hypothesis and conclude that the variance among the burn severity groups IS equal.

```{r DOC levenes test for burn severity}
# Null: the variance amoung the groups is equal. 
# conduct Levene's Test for equality of variances
leveneTest(Effect_size ~ burn_severity_category, data = doc_es_dnbr) #F-value: 0.365, p-value: 0.6977, Df: 2, 26

# plot 

boxplot(Effect_size ~ burn_severity_category,
  data = doc_es_dnbr,
  main = "Effect size Distribution by Burn Severity - DOC",
  xlab = "Burn Severity",
  ylab = "Effect Size",
  col = "#00AFBB",
  border = "black")
...
# Calculate percentages of each category
percentage_data <- doc_es_dnbr %>%
  count(burn_severity_category) %>%
  mutate(percentage = n / sum(n) * 100)

```
The p-value of the test is 0.6977, which is greater than our significance level of 0.05. 
Thus, we DO NOT reject the null hypothesis and conclude that the variance among the burn severity groups IS equal.
