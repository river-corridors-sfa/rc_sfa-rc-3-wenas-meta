...
### Building Random forest dataframe 
```{r }
# The effect sizes of each study's watershed
effect_size <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_Size.csv"), na = c('-9999', 'N/A')) %>% dplyr::select(Study_ID, response_var, Climate, Biome, Effect_size, Effect_size_pair, Time_Since_Fire)
...
unique(no_geo_random_forest_data$Site) # Supposed to be 33 sites here
...
summary(doc_tsf_mixed) # df: 40.9851, t-value: -4.792, p-value: 2.47e-05
fixef(doc_tsf_mixed)
...
summary(doc_tsf_cat) # df: 40.9851, t-value: -4.792, p-value: 2.47e-05
                            

# post hoc test 
emmeans::lsmeans(doc_tsf_cat, pairwise ~ TSF_grouped, adjust = "tukey") # across all time since fire and all burn percentages we dont see any climates that are significantly different from each other. 
