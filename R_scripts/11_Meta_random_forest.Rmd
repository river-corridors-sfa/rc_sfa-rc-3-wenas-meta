...
# Plotting
ggplot(filtered_df, aes(mean_fi * 100, reorder(predictor, mean_fi), fill = predictor)) + 
  geom_col(alpha = 0.8, show.legend = F, width = 0.7) +
  geom_errorbar(aes(xmin = min_fi * 100, xmax = max_fi * 100, width = 0.2)) +
  facet_wrap(~analyte, nrow = 1, labeller = labeller(analyte = dose.labs)) + 
  scale_fill_manual(values = var_colors) + 
...
## Take 2 ##
# Calculate the white noise mean_fi for each analyte
white_noise_values <- df %>% 
  filter(predictor == "white_noise") %>% 
  select(analyte, mean_fi) %>%
  rename(white_noise_fi = mean_fi)

# Filter the data to include only variables with mean_fi greater than the white noise value for each analyte
filtered_df <- df %>%
  inner_join(white_noise_values, by = "analyte") %>%
  filter(mean_fi > white_noise_fi) %>%
  select(-white_noise_fi)

# Plotting
ggplot(filtered_df, aes(mean_fi * 100, reorder(predictor, mean_fi), fill = predictor)) + 
  geom_col(alpha = 0.8, show.legend = F, width = 0.7) +
  geom_errorbar(aes(xmin = min_fi * 100, xmax = max_fi * 100, width = 0.2)) +
  facet_wrap(~ analyte, nrow = 1, scales = "free_y", labeller = labeller(analyte = dose.labs)) + 
  scale_fill_manual(values = var_colors) + 
  labs(x = "Feature Importance (%)", y = "", fill = "") +
  scale_y_discrete(labels = c(
    "bfi_ws" = "Baseflow Index",
