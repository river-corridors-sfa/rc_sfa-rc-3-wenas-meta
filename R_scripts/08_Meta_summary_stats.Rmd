...
landcover_data <- read_csv(here("Output_for_analysis", "06_Meta_geospatial_extraction_with_comids",
  ...
  select(Site, comid, PctOw2001:PctHbWet2019)
...
landcover_data_merged <- left_join(no_geo_random_forest_data, landcover_long) %>% 
...
random_forest_data <- random_forest_data %>% 
  group_by(Site, landcover_type) %>% 
  distinct() %>% 
  ungroup()

# Making wide so the landcover types are in columns 
random_forest_data_wide <- random_forest_data %>% 
  pivot_wider(
    names_from = "landcover_type",
    values_from = "percent_landcover"
  )

#splitting by analyte
doc_RF <- random_forest_data_wide %>%
  filter(response_var == "DOC_Interp")

write_csv(doc_RF, here("Output_for_analysis", "08_Meta_summary_stats", "doc_meta_random_forest.csv"))

no3_RF <- random_forest_data_wide %>%
...
doc_correlation <- doc_RF %>% 
  dplyr::select(Effect_size, totdasqkm, minelevraw, maxelevsmo, minelevsmo, elevfixed, slope, Tmax9120Ws,Tmean9120Ws, Tmin9120Ws, OmWs, BFIWs, PermWs, RunoffWs, burn_percentage, Time_Since_Fire) %>% 
...
# Specify the file path and format for the output PDF file
output_file <- "~/GitHub/rc_sfa-rc-3-wenas-meta/initial_plots/08_Meta_summary_stats/NO3_geospatial_correlation.pdf"
