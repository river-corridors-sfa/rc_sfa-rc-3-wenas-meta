...
# # of DOC and NO3 samples pulled for each study
  # How many samples are available each year 
meta_df_long <- meta_df %>% 
  select(Study_ID, Pair, Site, Sampling_Date, Time_Since_Fire, DOC, NO3) %>% 
  filter(Study_ID != "Murphy et al. 2015")

meta_df_long <- meta_df_long %>%
  pivot_longer(
    cols = c(DOC, NO3),
    names_to = "analyte",
    values_to = "value",
...
summary_df_long <- meta_df_long %>% 
  group_by(Study_ID, analyte) %>% 
  summarise(n = n())

unique_long <- meta_df_long %>% 
  group_by(Study_ID, analyte, Pair) %>% 
  distinct(Sampling_Date, .keep_all = TRUE)

summary_unique <- unique_long %>% 
  group_by(Study_ID, analyte) %>% 
  summarise(n = n())

summary_tsf <- unique_long %>% 
  group_by(Study_ID, analyte, Time_Since_Fire) %>% 
  summarise(n = n())

summary_site_tsf <- unique_long %>% 
  group_by(Study_ID, Site, analyte, Time_Since_Fire) %>% 
  summarise(n = n())

write_csv(summary_site_tsf, here("Output_for_analysis", "08_Meta_summary_stats", "meta_summary.csv"))
...
### Mast & Clow; 2008
This chunk runs 100 iterations of the down-selected approach and then plots the upper and lower bounds time series with the real data overlain that we will use in the supplemental 
```{r}
...
# NO3
ggplot(combined_df_filled) +
  geom_density(aes(x = NO3_Interp, group = iteration), alpha = 0.001) +
  theme_minimal()
...
summary_stats <- combined_df_filled %>%
  ...
  summarize(mean_DOC_Interp = mean(DOC_Interp, na.rm = TRUE),
            ...
            mean_NO3_Interp = mean(NO3_Interp, na.rm = TRUE),
            sd_NO3_Interp = sd(NO3_Interp, na.rm = TRUE))

summary_stats <- summary_stats %>%
  mutate(lower_bound_DOC = mean_DOC_Interp - sd_DOC_Interp,
         upper_bound_DOC = mean_DOC_Interp + sd_DOC_Interp,
         lower_bound_NO3 = mean_NO3_Interp - sd_NO3_Interp,
         upper_bound_NO3 = mean_NO3_Interp + sd_NO3_Interp)

# DOC
ggplot(combined_df_filled, aes(x = Sampling_Date)) +
  geom_ribbon(data = summary_stats, aes(ymin = lower_bound_DOC, ymax = upper_bound_DOC), fill = "grey80", alpha = 0.5) +
...
ggsave("mast_ds_discrete_iteration.pdf",
       path = here("initial_plots", "15_Meta_calculate_effect_sizes_papers"),
       width = 6, height = 4, units = "in")

# NO3
ggplot(combined_df_filled, aes(x = Sampling_Date)) +
  geom_ribbon(data = summary_stats, aes(ymin = lower_bound_NO3, ymax = upper_bound_NO3), fill = "grey80", alpha = 0.5) +
  geom_line(data = summary_stats, aes(y = mean_NO3_Interp), color = "blue", size = 1) +
  labs(title = "Time Series of NO3 Interpolated Values with Distribution Ribbon",
       x = "Sampling Date",
       y = "Interpolated NO3 Values (mg_N/L)") +
  theme_bw()

# Select columns of interest
mast_discrete_NO3 <- mast_discrete_units %>%
  filter(Study_ID == "Mast & Clow; 2008") %>% 
  select(Study_ID, Pair, Site, Sampling_Date, NO3_mg_N_L) %>% 
  na.omit()
...
ggplot(combined_df_filled, aes(x = Sampling_Date)) +
  geom_ribbon(data = summary_stats, aes(ymin = lower_bound_NO3, ymax = upper_bound_NO3), fill = "grey80", alpha = 0.5) +
  geom_line(data = summary_stats, aes(y = mean_NO3_Interp), color = "blue", size = 1) +
  geom_point(data = mast_discrete_NO3, aes(y = NO3_mg_N_L), color = "red", size = 1, alpha = 1.5) +
  labs(title = "Time Series of NO3 Interpolated Values with Distribution Ribbon",
       x = "Sampling Date",
       y = "Interpolated NO3 Values (mg_N/L)") +
  theme_bw()

ggsave("mast_ds_discrete_iteration_no3.pdf",
       path = here("initial_plots", "15_Meta_calculate_effect_sizes_papers"),
       width = 6, height = 4, units = "in")
...
# Load in Effect Size for Mast that was for full gap filling
```{r}
# Read in the old effect size data frame
effect_size_fill <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_Size.csv"))
...
# Calculate Effect Size for the 100-iteration down-selecting for Mast & Clow
```{r}
# add watershed area back in to calculate effect size 
combined_df_filled <- combined_df_filled %>% 
  mutate(Area_watershed_km = case_when(
      Site == 'Pinchot' ~ 47.3,
      Site == 'Coal' ~ 96.4
    ))

# Pivot longer to get DOC and NO3 in one column
iteration_long <- combined_df_filled %>%
  pivot_longer(
    cols = DOC_Interp:NO3_Interp,
    names_to = "response_var",
    values_to = "concentration",
    values_drop_na = TRUE
  )

# Calculate temporal normalization for each iteration 
yearly_iteration <- iteration_long %>%
  group_by(iteration, Study_ID, response_var, Pair, year = year(Sampling_Date)) %>% 
  mutate(meanConc = mean(concentration, na.rm = TRUE),
         TempNorm = meanConc / Area_watershed_km) %>% 
  select(iteration, Study_ID, response_var, Pair, year, meanConc, TempNorm)

# Create control_treatment data frame structure
control_treatment <- yearly_iteration %>%
  group_by(iteration, Study_ID, response_var, Pair, year, Burn_Unburn = ifelse(Pair == "Control", "Unburn", "Burn")) %>%
  summarize(Xe = sum(TempNorm[Burn_Unburn == "Burn"], na.rm = TRUE),
            Xc = sum(TempNorm[Burn_Unburn == "Unburn"], na.rm = TRUE), .groups = 'drop')

# Make data frame wide so we can divide by columns 
control_treatment_wide <- control_treatment %>%
  group_by(iteration, Study_ID, response_var, year) %>%
  summarize(
...
effect_size_iteration <- control_treatment_wide %>%
  group_by(iteration, Study_ID, response_var, year) %>%
  mutate(lnR1 = log(Xe1 / Xc)) %>%
  summarize(across(starts_with("lnR"), mean, na.rm = TRUE), .groups = 'drop')


```

# Compare the effect sizes for each iteration and the full fill
```{r}
# Ensure the appropriate columns are factors
effect_size_iteration$response_var <- as.factor(effect_size_iteration$response_var)

mast_es_fill$response_var <- as.factor(mast_es_fill$response_var)

# Merge data frames
iteration_filled_es <- effect_size_iteration %>%
  inner_join(mast_es_fill, by = c("Study_ID", "response_var", "year")) %>%
  rename(Effect_size1 = lnR1,
         Effect_size2 = Effect_size)

library(broom)

# Function to perform linear regression and extract R-squared value
get_r_squared <- function(df) {
  model <- lm(Effect_size2 ~ Effect_size1, data = df)
  summary(model)$r.squared
}

# Apply the function to each iteration
r_squared_values <- iteration_filled_es %>%
  group_by(response_var, iteration) %>%
  summarize(r_squared = get_r_squared(cur_data()), .groups = 'drop')

# Calculate average and standard deviation of R-squared values
r_squared_summary <- r_squared_values %>%
  group_by(response_var) %>% 
  summarize(mean_r_squared = mean(r_squared, na.rm = TRUE),
            sd_r_squared = sd(r_squared, na.rm = TRUE))

print(r_squared_summary)

# Calculate mean and confidence interval
mean_r_squared <- mean(r_squared_values$r_squared, na.rm = TRUE)

ci_r_squared <- qt(0.975, df = nrow(r_squared_values) - 1) * sd(r_squared_values$r_squared) / sqrt(nrow(r_squared_values))

ggplot(r_squared_values, aes(x = "", y = r_squared)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, alpha = 0.5) +
  labs(title = "R-squared Values Across Iterations",
       x = "",
       y = "R-squared Value") +
  theme_bw()

median(r_squared_values$r_squared)


# TAKING INTO ACCOUNT DOC AND NO3
# Calculate mean and confidence interval for each response variable
...
# Merge data frames
merged_data <- effect_size_iteration %>%
  inner_join(mast_es_fill, by = c("Study_ID", "response_var", "year")) %>%
  rename(Effect_size1 = lnR1, Effect_size2 = Effect_size)

# Function to perform linear regression and extract R-squared value
get_r_squared <- function(df) {
  model <- lm(Effect_size1 ~ Effect_size2, data = df)
  summary(model)$r.squared
}

options(scipen = 999)
# Apply the function to each iteration and each response_var
r_squared_values <- merged_data %>%
  group_by(iteration, response_var) %>%
  summarize(r_squared = get_r_squared(cur_data()), .groups = 'drop')
...
r_squared_summary <- r_squared_values %>%
  ...
  summarize(
    ...
    se_r_squared = sd(r_squared, na.rm = TRUE) / sqrt(n()),
    ci_lower = mean_r_squared - qt(0.975, df = n() - 1) * se_r_squared,
    ci_upper = mean_r_squared + qt(0.975, df = n() - 1) * se_r_squared
  )
  
# Print the summary
print(r_squared_summary)

# Plot R-squared values using boxplot with geom_jitter and error bars
ggplot(r_squared_values, aes(x = response_var, y = r_squared)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, alpha = 0.5) +
  labs(title = "Distribution of R-squared Values Across Iterations",
       x = "Response Variable",
       y = "R-squared Value") +
  theme_minimal()

# ggsave("mast_r2_ws.pdf",
#        path = here("initial_plots", "15_Meta_calculate_effect_sizes_papers"),
#        width = 6, height = 4, units = "in")
...
# ======================================================================
#### STEP 3 ###
Plot the gap filled data on top of the interpolated data for each study
...
# Load in gap-filled data
```{r}
# Gap-fill interpolated data
gap_fill <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "gap_fill.csv"))

# Define the directory where the plots will be saved
output_directory <- "~/GitHub/rc_sfa-rc-3-wenas-meta/initial_plots/15_Meta_calculate_effect_sizes_papers/gap_fill/DOC"

# Loop over each unique Study_ID
unique_study_ids <- unique(gap_fill$Study_ID)

for(study_id in unique_study_ids) {
  # Subset the data for the current Study_ID
  subset_gap_fill <- gap_fill %>% filter(Study_ID == study_id) %>% 
    filter(Pair %in% c("Control", "Control_1", "Control_2", "Site_1", "Site_2", "Site_3"))
  subset_control_impact_units <- control_impact_units %>% filter(Study_ID == study_id) %>% 
    filter(Pair %in% c("Control", "Control_1", "Control_2", "Site_1", "Site_2", "Site_3"))
  
      # Modify DOC_Interp values for specific Study_ID and time windows
  if (study_id == "Hickenbottom et al. 2023") {
    subset_gap_fill <- subset_gap_fill %>%
      mutate(DOC_Interp = ifelse(
        (Pair == "Control_1" & Sampling_Date >= as.Date("2021-10-06") & Sampling_Date <= as.Date("2022-04-18")) |
        (Pair == "Site_1" & Sampling_Date >= as.Date("2022-04-01") & Sampling_Date <= as.Date("2022-07-02")) |
        (Pair == "Site_1" & Sampling_Date >= as.Date("2022-09-20") & Sampling_Date <= as.Date("2023-04-04")),
        NA,
        DOC_Interp
      ))
  }
  
  # Check if there are any non-NA DOC values before plotting
  if (all(is.na(subset_gap_fill$DOC_Interp)) && all(is.na(subset_control_impact_units$DOC_mg_C_L))) {
    next  # Skip this iteration of the loop if all DOC values are NA
  }
  
  # Create the plot
  plot <- ggplot() +
    geom_point(data = subset_gap_fill, aes(x = Sampling_Date, y = DOC_Interp), color = "grey", alpha = 0.7) +
    geom_point(data = subset_control_impact_units, aes(x = Sampling_Date, y = DOC_mg_C_L), color = "red", size = 1.5) +
    facet_wrap(~Pair, scales = "free") +
    labs(title = paste(study_id, "DOC Data Comparison"),
         x = "Sampling Date",
         y = "DOC Concentration",
         color = "Data Source") +
    theme_bw()
  ...
  # Save the plot
  ggsave(filename = file.path(output_directory, paste0(study_id, "_DOC_gap_fill_plot.png")), plot = plot, width = 10, height = 7)
}
...
# Doesn't work:
  # Coombs & Melack - doesnt have DOC
  # Gerla & Galloway - doesnt have DOC
  # Gluns & Toews - doesnt have DOC
  # Hauer & Spencer - doesnt have DOC
  # Neary & Currier - doesnt have DOC
  # Oliver - doesnt have DOC
  # Tiedemann - doesnt have DOC
...
```{r}
# Define the directory where the plots will be saved
output_directory <- "~/GitHub/rc_sfa-rc-3-wenas-meta/initial_plots/15_Meta_calculate_effect_sizes_papers/gap_fill/NO3"
...
for(study_id in unique_study_ids) {
  ...
  # Check if there are any non-NA DOC values before plotting
  if (all(is.na(subset_gap_fill$NO3_Interp)) && all(is.na(subset_control_impact_units$NO3_mg_N_L))) {
    next  # Skip this iteration of the loop if all DOC values are NA
  }
  
  # Create the plot
  plot <- ggplot() +
    geom_point(data = subset_gap_fill, aes(x = Sampling_Date, y = NO3_Interp), color = "grey", alpha = 0.7) +
    geom_point(data = subset_control_impact_units, aes(x = Sampling_Date, y = NO3_mg_N_L), color = "red", size = 1.5) +
    facet_wrap(~Pair, scales = "free") +
    labs(title = paste(study_id, "NO3 Data Comparison"),
         x = "Sampling Date",
         y = "NO3 Concentration",
         color = "Data Source") +
    theme_bw()
  ...
  # Save the plot
  ggsave(filename = file.path(output_directory, paste0(study_id, "_NO3_gap_fill_plot.png")), plot = plot, width = 10, height = 7)
}
...
# Hickenbottom TEST
```{r}
# Plot Hickenbottom to see if it works
hicken_interp <- gap_fill %>% 
  filter(Study_ID == "Hickenbottom et al. 2023")

#plot
ggplot(hicken_interp) +
  geom_line(aes(x = Sampling_Date, y = DOC_Interp), color = "grey") +
  facet_wrap(~Pair, scales = "free") +
  theme_bw()

# observed
hicken_no_gap <- meta_df %>% 
  filter(Study_ID == "Hickenbottom et al. 2023") %>% 
  mutate(Sampling_Date = ymd(Sampling_Date))

# Plot no gap fill
ggplot(hicken_no_gap) +
  geom_point(aes(x = Sampling_Date, y = DOC, color = Pair)) +
  facet_wrap(~Pair, scales = "free") +
  theme_bw()

# looks like there is a gap of more than 50 days for Hickenbottom. Lets fill and remove interpolated data that is greater than 50 days. 
hicken_impact_units <- control_impact_units %>% 
   filter(Study_ID == "Hickenbottom et al. 2023")

hicken_impact_fill <- hicken_impact_units %>%
  ...
  mutate(
     # Calculate the difference in days between consecutive sampling dates
    day_diff = c(0, diff(Sampling_Date)),
  # Create a group identifier based on gaps greater than 50 days
    fill_group = (case_when(day_diff > 50 ~ 1, TRUE ~ 0))
  ) %>%
  group_by(Study_ID, Site, fill_group) %>%
  # Now fill in the missing dates only within valid groups
  complete(Sampling_Date = seq(min(Sampling_Date), max(Sampling_Date), by = "1 day")) %>%
  fill(Study_ID:Burn_Unburn) %>%
  ungroup() %>%
  group_by(Site, fill_group) %>%
  mutate(
    # Apply interpolation within groups where the dates are not too far apart
    NO3_Interp = na.approx(NO3_mg_N_L, na.rm = FALSE),
    DOC_Interp = na.approx(DOC_mg_C_L, na.rm = FALSE)
  ) %>%
  ungroup() %>%
  fill(Study_ID:Burn_Unburn) %>%
  select(-day_diff, -fill_group)  # Optionally remove intermediate columns


# Plot to see if this worked:
ggplot(hicken_impact_fill) +
  geom_line(aes(x = Sampling_Date, y = DOC_Interp), color = "grey") +
  facet_wrap(~Pair, scales = "free") +
  theme_bw()
...
hicken_impact_fill <- hicken_impact_units %>%
  ...
  mutate(
    ...
    day_diff = c(0, diff(Sampling_Date)),
    # Create a group identifier based on gaps greater than 50 days
    is_big_gap = (case_when(day_diff > 50 ~ 1, TRUE ~ 0))
  ) %>%
  group_by(Study_ID, Site) %>%
  mutate(
    fill_group = cumsum(is_big_gap)
  ) %>%
  group_by(Study_ID, Site, fill_group) %>%
  # Fill in the missing dates within each valid group
  complete(Sampling_Date = seq(min(Sampling_Date), max(Sampling_Date), by = "1 day")) %>%
  fill(Study_ID:Burn_Unburn) %>%
  ungroup() %>%
  group_by(Site, fill_group) %>%
  mutate(
    # Apply interpolation only for groups without big gaps (where fill_group is 0)
    NO3_Interp = if_else(fill_group == 0, na.approx(NO3_mg_N_L, na.rm = FALSE), as.numeric(NA)),
    DOC_Interp = if_else(fill_group == 0, na.approx(DOC_mg_C_L, na.rm = FALSE), as.numeric(NA))
  ) %>%
  ungroup() %>%
  fill(Study_ID:Burn_Unburn) %>%
  select(-day_diff, -is_big_gap, -fill_group)  # Optionally remove intermediate columns
...
hicken_impact_fill <- hicken_impact_units %>%
  ...
  mutate(
    ...
    # Create a group identifier based on gaps greater than 50 days
    is_big_gap = case_when(day_diff > 50 ~ 1, TRUE ~ 0),
    # A running count of valid fill groups
    fill_group = cumsum(is_big_gap)
  ) %>%
  # Complete the sequence of missing dates within each valid group
  group_by(Study_ID, Site, fill_group) %>%
  complete(Sampling_Date = seq(min(Sampling_Date), max(Sampling_Date), by = "1 day")) %>%
  ungroup() %>%
  group_by(Study_ID, Site) %>%
  fill(Study_ID:Burn_Unburn) %>%
  ungroup() %>%
  # Interpolate values only within valid groups, ensuring we reset where necessary
  group_by(Study_ID, Site, fill_group) %>%
  mutate(
    NO3_Interp = if_else(max(is_big_gap) == 1, as.numeric(NA), na.approx(NO3_mg_N_L, na.rm = FALSE)),
    DOC_Interp = if_else(max(is_big_gap) == 1, as.numeric(NA), na.approx(DOC_mg_C_L, na.rm = FALSE))
  ) %>%
  ungroup() %>%
  fill(Study_ID:Burn_Unburn) %>%
  select(-day_diff, -is_big_gap, -fill_group)  # Optionally remove intermediate columns

# Display the structure of the final result to verify the outcome
print(head(hicken_impact_fill))
...
# Burd TEST
```{r}
# Plot Burd to see if it works
burd_interp <- gap_fill %>% 
  filter(Study_ID == "Burd et al 2018")

# Filter out Burd observed points
burd_no_gap <- meta_df %>% 
  filter(Study_ID == "Burd et al 2018") %>% 
  mutate(Sampling_Date = ymd(Sampling_Date))
...
# Plot on top of each other
ggplot() +
  geom_point(data = burd_interp, aes(x = Sampling_Date, y = DOC_Interp), alpha = 0.5, color = "grey") +
  geom_point(data = burd_no_gap, aes(x = Sampling_Date, y = DOC), size = 1.5, color = "red") +
  facet_wrap(~Pair, scales = "free") +
  theme_bw()
...
```{r}
# Merge the data frames 
merged_effect_size <- left_join(effect_size_fill, downselected_es, 
        by = c("Study_ID", "response_var", "year", "Effect_size_pair"),
...
############################# SCRIPT GRAVEYARD ###############################
Coombs & Melack study is in here because it is a time series of a week worth of data so there really isnt a data interpolation. 
### Coombs & Melack ###
This will be the same workflow as above that we did for Mast
```{r}
coombs_long <- control_long %>% 
...
# Combine the results into one data frame with an additional column to distinguish the iterations
combined_df_filled <- bind_rows(downselected_filled_list, .id = 'iteration') %>% select(iteration, Study_ID, Pair, Site, Sampling_Date, NO3_Interp)

# Write out as csv 
# write_csv(combined_df_filled, here("initial_plots", "15_Meta_calculate_effect_sizes_papers", "coombs_iteration.csv"))

# Compute the mean values for each iteration
mean_df <- combined_df_filled %>%
  group_by(iteration, Pair) %>%
  summarize(mean_NO3_Interp = mean(NO3_Interp, na.rm = TRUE))
...
# Calculate mean and standard deviation for each date
summary_stats <- combined_df_filled %>%
  group_by(Sampling_Date, Pair) %>%
  summarize(mean_NO3_Interp = mean(NO3_Interp, na.rm = TRUE),
            sd_NO3_Interp = sd(NO3_Interp, na.rm = TRUE))

summary_stats <- summary_stats %>%
  mutate(lower_bound = mean_NO3_Interp - sd_NO3_Interp,
         upper_bound = mean_NO3_Interp + sd_NO3_Interp)
...
### Now I want to plot the actual data on top 
# extract discrete observations from the Mast & Clow paper 2008.
coombs_discrete <- control_long %>% 
  filter(Study_ID == "Coombs & Melack; 2013")

# Change the units
coombs_discrete_units <- coombs_discrete %>% 
  mutate(
    # Standardize Area_watershed to kilometers
    Area_watershed_km = case_when(
      Area_unit == 'ha' ~ Area_watershed * 0.01,
...
# Select columns of interest
coombs_discrete_NO3 <- coombs_discrete_units %>%
  filter(Study_ID == "Coombs & Melack; 2013") %>% 
  select(Study_ID, Pair, Site, Sampling_Date, NO3_mg_N_L) %>% 
  na.omit()
...
ggplot(combined_df_filled, aes(x = Sampling_Date)) +
  geom_ribbon(data = summary_stats, aes(ymin = lower_bound, ymax = upper_bound), fill = "grey80", alpha = 0.5) +
  geom_line(data = summary_stats, aes(y = mean_NO3_Interp), color = "blue", size = 1) +
  geom_point(data = coombs_discrete_NO3, aes(y = NO3_mg_N_L), color = "red", size = 1, alpha = 1.5) +
  facet_wrap(~Pair, scales = "free") +
  labs(title = "Time Series of NO3 Interpolated Values with Distribution Ribbon",
       x = "Sampling Date",
       y = "Interpolated NO3 Values (mg_C/L)") +
  theme_bw()
