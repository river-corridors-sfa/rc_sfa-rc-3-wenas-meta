...
# This is doing the same thing as chunck before where we are creating a column header for each individual site but doing it for the studies that have multiple controls. 
no_watershed_abbott <- yearly_mean_site %>%
  mutate(meanConc = as.numeric(meanConc))



abbott_test_no_watershed <- no_watershed_abbott %>%
  group_by(Study_ID, Pair, year, Burn_Unburn, response_var, Climate) %>%
...
yearly_multiple_control_no_watershed <- abbott_test_no_watershed %>% 
  group_by(Study_ID, response_var, Climate, year) %>%
  summarize(Xe1 = sum(Xe[Pair == "Site_1"], na.rm = TRUE),
            Xc = sum(Xc[Pair == "Control"], na.rm = TRUE))
...
write_csv(yearly_effect_size_combine_long_no_watershed, here("Output_for_analysis", "02_Abbott_effect_size", "Abbott_effect_size_no_watershed.csv"))
...
ES_orig <- read_csv(here("Output_for_analysis", "02_Abbott_effect_size", "Abbott_effect_size.csv"))

ggplot() +
  geom_point(data = ES_no_watershed, aes(x = as.character(year), y = Effect_size_no_watershed), color = "red") +
  geom_point(data = ES_orig, aes(x = as.character(year), y = Effect_size), color = "black") +
