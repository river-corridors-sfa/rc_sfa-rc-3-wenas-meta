...
getNHDcomid <- function(df = sites){
  # Select NHD flowlines that df are located on
  subset_nhdplus(comids = df$comid,
                 output_file = 'data/site_flowlines.gpkg',
                 nhdplus_data = 'download',
                 overwrite = TRUE,
  ...
  # Link NHD HUC data (UPDATED: get_huc12 -> get_huc)
  site_hucs <- list()
  for(i in 1:nrow(coords)){
    site_hucs[[i]] <- get_huc(coords[i,], type = "huc12", t_srs = 4269)
  }
