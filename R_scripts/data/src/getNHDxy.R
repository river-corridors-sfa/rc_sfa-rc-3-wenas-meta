...
#                output_file = 'R_scripts/data/site_flowlines.gpkg',
...
#                out_prj = 4269)
# 
# site_lines <- st_read('R_scripts/data/site_flowlines.gpkg', quiet = T) %>%
...
getNHDxy <- function(df = sites){
  # Get the comid for each site
  for(i in 1:nrow(df)){
    df$comid[i] <- discover_nhdplus_id(df[i,])
  }
  
  # Link NHD HUC data to each sample (UPDATED: get_huc12 -> get_huc)
  site_hucs <- list()
  for(i in 1:nrow(df)){
    site_hucs[[i]] <- get_huc(df[i,], type = "huc12", t_srs = 4269)
  }
