...
# ## 10/1/2025 - generate a data frame that doesn't normalize by watershed area ##
# no_area <- metadata_long %>%
#   group_by(Study_ID, response_var, Pair, year) %>% 
#   summarize(meanConc = mean(concentration)) %>% 
#   left_join(site_data) %>% 
#   group_by(Pair, year) %>%
...
#### EFFECT SIZE without watershed area - 10/1/2025
```{r}
# Bringing the columns back up to match to be able to calculate
no_watershed <- yearly_mean_site %>%
  mutate(meanConc = as.numeric(meanConc))  # Taking out Hauer and Spencer and Hickenbottom because they have mutliple controls and that is different than the rest of these sites

# This is doing the same thing as chunck before where we are creating a column header for each individual site but doing it for the studies that have multiple controls. 
no_watershed_crandall <- yearly_mean_site %>%
  mutate(meanConc = as.numeric(meanConc))



crandall_test_no_watershed <- no_watershed_crandall %>%
  group_by(Study_ID, Pair, year, Burn_Unburn, response_var, Climate) %>%
...
yearly_multiple_control_no_watershed <- crandall_test_no_watershed %>% 
  group_by(Study_ID, response_var, Climate, year) %>%
  summarize(Xe1 = sum(Xe[Pair == "Site_1"], na.rm = TRUE),
            Xe2 = sum(Xe[Pair == "Site_2"], na.rm = TRUE),
            Xe3 = sum(Xe[Pair == "Site_3"], na.rm = TRUE),
...
# pivoting to make the responses in one column
# effect size 
yearly_effect_size_combine_long_no_watershed <- yearly_effect_multiple_no_watershed %>% 
  pivot_longer(
    cols = lnR1:lnR4,
    names_to = "Effect_size_pair",
...
write_csv(yearly_effect_size_combine_long_no_watershed, here("Output_for_analysis", "01_Crandall_effect_size", "Crandall_effect_size_no_watershed.csv"))


```

### EFFECT SIZE COMP BETWEEN NORMALIZATION AND NOT ###
```{r}
ES_no_watershed <- yearly_effect_size_combine_long_no_watershed

ES_orig <- read_csv(here("Output_for_analysis", "01_Crandall_effect_size", "Crandall_effect_size.csv"))

ggplot() +
  geom_point(data = ES_no_watershed, aes(x = Effect_size_pair, y = Effect_size_no_watershed), color = "red") +
  geom_point(data = ES_orig, aes(x = Effect_size_pair, y = Effect_size), color = "black") +
