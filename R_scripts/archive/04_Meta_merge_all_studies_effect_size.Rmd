...
# Gap Filling - 50 days
```{r}
# Step 1: Prepare the Data and Calculate Days Between Samples
control_impact_units <- control_impact_units %>%
  ...
  mutate(Days_Between_Samples = c(NA, diff(Sampling_Date)),
         Large_Gap_Flag = ifelse(Days_Between_Samples > 50, TRUE, FALSE))


# Step 2: Create a Complete Daily Time Series for Each Study_ID and Site
create_time_series <- function(df) {
  all_dates <- tibble(Sampling_Date = seq(min(df$Sampling_Date, na.rm = TRUE), max(df$Sampling_Date, na.rm = TRUE), by = "day"))
  merged_data <- all_dates %>% left_join(df, by = "Sampling_Date")
  return(merged_data)
}
...
# Step 3: Fill the Large_Gap_Flag Based on Surrounding Values
fill_gap_flags <- function(data) {
  data <- data %>%
    arrange(Sampling_Date) %>%
    mutate(Large_Gap_Flag = zoo::na.locf(Large_Gap_Flag, na.rm = FALSE, fromLast = TRUE),
...
# Step 4: Interpolate Values Based on the Updated Large_Gap_Flag
interpolate_values_based_on_flag <- function(data, analyte) {
  data <- data %>%
    arrange(Sampling_Date) %>%
    mutate(!!paste0(analyte, "_Interp") := ifelse(Large_Gap_Flag, NA, zoo::na.approx(get(analyte), na.rm = FALSE, maxgap = 50)))
  return(data)
}
...
# Rename columns to fit downstream workflow
daily_time_series <- daily_time_series %>% 
  rename(DOC_Interp = "DOC_mg_C_L_Interp",
         NO3_Interp = "NO3_mg_N_L_Interp")

# Filter to columns of interest
daily_time_series_filter <-  daily_time_series %>% 
  dplyr::select("Study_ID", "latitude", "longitude", "Area_watershed_km", "Pair", "Climate", "Site", "Burn_Unburn", "Sampling_Date", "DOC_mg_C_L", "NO3_mg_N_L", "DOC_Interp", "NO3_Interp", "year", "month", "day")

# Fill
daily_time_series_filter <- daily_time_series_filter %>% 
  fill(latitude, longitude, Area_watershed_km, Climate, Burn_Unburn)

write_csv(daily_time_series_filter, here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "gap_fill.csv"))
...
control_impact_units <- control_impact_units %>%
  ...
  arrange(Sampling_Date) %>%
  mutate(Days_Between_Samples = c(NA, diff(Sampling_Date)),
         Large_Gap_Flag = ifelse(Days_Between_Samples > 10, TRUE, FALSE))
...
interpolate_values_based_on_flag <- function(data, analyte) {
  data <- data %>%
    arrange(Sampling_Date) %>%
    mutate(!!paste0(analyte, "_Interp") := ifelse(Large_Gap_Flag, NA, zoo::na.approx(get(analyte), na.rm = FALSE, maxgap = 10)))
  return(data)
}
...
write_csv(daily_time_series_filter, here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "gap_fill_10.csv"))
...
control_impact_units <- control_impact_units %>%
  ...
  arrange(Sampling_Date) %>%
  mutate(Days_Between_Samples = c(NA, diff(Sampling_Date)),
         Large_Gap_Flag = ifelse(Days_Between_Samples > 20, TRUE, FALSE))
...
interpolate_values_based_on_flag <- function(data, analyte) {
  data <- data %>%
    arrange(Sampling_Date) %>%
    mutate(!!paste0(analyte, "_Interp") := ifelse(Large_Gap_Flag, NA, zoo::na.approx(get(analyte), na.rm = FALSE, maxgap = 20)))
  return(data)
}
...
write_csv(daily_time_series_filter, here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "gap_fill_20.csv"))
...
control_impact_units <- control_impact_units %>%
  ...
  arrange(Sampling_Date) %>%
  mutate(Days_Between_Samples = c(NA, diff(Sampling_Date)),
         Large_Gap_Flag = ifelse(Days_Between_Samples > 30, TRUE, FALSE))
...
interpolate_values_based_on_flag <- function(data, analyte) {
  data <- data %>%
    arrange(Sampling_Date) %>%
    mutate(!!paste0(analyte, "_Interp") := ifelse(Large_Gap_Flag, NA, zoo::na.approx(get(analyte), na.rm = FALSE, maxgap = 30)))
  return(data)
}
...
write_csv(daily_time_series_filter, here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "gap_fill_30.csv"))
...
control_impact_units <- control_impact_units %>%
  ...
  arrange(Sampling_Date) %>%
  mutate(Days_Between_Samples = c(NA, diff(Sampling_Date)),
         Large_Gap_Flag = ifelse(Days_Between_Samples > 40, TRUE, FALSE))
...
interpolate_values_based_on_flag <- function(data, analyte) {
  data <- data %>%
    arrange(Sampling_Date) %>%
    mutate(!!paste0(analyte, "_Interp") := ifelse(Large_Gap_Flag, NA, zoo::na.approx(get(analyte), na.rm = FALSE, maxgap = 40)))
  return(data)
}
...
write_csv(daily_time_series_filter, here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "gap_fill_40.csv"))
...
metadata_long <- daily_time_series_filter %>%
  pivot_longer(
...
#### EFFECT SIZE for watershed normalized by area
...
yearly_effect_size_combine_long <-yearly_effect_size_combine_long %>% 
  add_column(Xc34 = NA) %>% 
  ungroup() %>% 
  dplyr::select(Study_ID, response_var, Climate, year, Xe1, Xe2, Xe3, Xe4, Xe5, Xc, Xc2, Xc34, Xc345, Biome, Effect_size_pair, Effect_size)
...
TSF_year <- control_impact %>% 
  mutate(year = year(Sampling_Date)) %>% 
  dplyr::select(Study_ID, Time_Since_Fire, year) %>% 
  distinct()
...
write_csv(yearly_effect_size_combine_long_out, here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_Size_40days.csv"))

unique(yearly_effect_size_combine_long_out$Study_ID)

```

####10/1/2025 - EFFECT SIZE without watershed area 
```{r}
# Taking out Hauer and Spencer and Hickenbottom because they have mutliple controls and that is different than the rest of these sites
no_watershed <- yearly_mean_site %>%
  filter(Study_ID != "Hauer & Spencer 1998", Study_ID != "Hickenbottom et al. 2023") %>%
  mutate(meanConc = as.numeric(meanConc)) 

write_csv(no_watershed, here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Watershed_area_normalized_vs_non_normalized_differences_in_nitrate_and_DOC.csv"))


# Creating a control and treatment column for the temporal normalization
yearly_mean_site_test_2_no_watershed <- no_watershed %>%
  group_by(Study_ID, Pair, year, Burn_Unburn, response_var, Climate) %>%
  dplyr::summarize(Xe = sum(meanConc[Burn_Unburn == "Burn"], na.rm = TRUE),
                   Xc = sum(meanConc[Burn_Unburn == "Unburn"], na.rm = TRUE)) 

yearly_mean_site_test_3_no_watershed <- yearly_mean_site_test_2_no_watershed %>% 
  group_by(Study_ID, response_var, Climate, year) %>%
  summarize(Xe1 = sum(Xe[Pair == "Site_1"], na.rm = TRUE),
            Xe2 = sum(Xe[Pair == "Site_2"], na.rm = TRUE),
            Xe3 = sum(Xe[Pair == "Site_3"], na.rm = TRUE),
...
# Calculating effect size: ln((MeanBurn/MeanUnburn)) for all the sites with only one control 
yearly_effect_size_no_watershed <- yearly_mean_site_test_3_no_watershed %>% 
  group_by(Study_ID, response_var, Climate, year) %>%
  mutate(lnR1 = log(Xe1/Xc),
         lnR2 = log(Xe2/Xc),
         lnR3 = log(Xe3/Xc),
...
# This is doing the same thing as chunk before where we are creating a column header for each individual site but doing it for the studies that have multiple controls. 
yearly_mean_hauer_hickenbottom_no_watershed <- yearly_mean_site %>%
  filter(Study_ID == "Hauer & Spencer 1998" | Study_ID == "Hickenbottom et al. 2023") %>%
  mutate(meanConc = as.numeric(meanConc))

hauer_hickenbottom_test_no_watershed <- yearly_mean_hauer_hickenbottom_no_watershed %>%
  group_by(Study_ID, Pair, year, Burn_Unburn, response_var, Climate) %>%
  dplyr::summarize(Xe = sum(meanConc[Burn_Unburn == "Burn"], na.rm = TRUE),
                   Xc = sum(meanConc[Burn_Unburn == "Unburn"], na.rm = TRUE)) # Creating a control and treatment column for the temporal normalization 

# Include in the no_watershed
no_watershed2 <- no_watershed %>% 
  rbind(yearly_mean_hauer_hickenbottom_no_watershed)


yearly_multiple_control_no_watershed <- hauer_hickenbottom_test_no_watershed %>% 
  group_by(Study_ID, response_var, Climate, year) %>%
  summarize(Xe1 = sum(Xe[Pair == "Site_1"], na.rm = TRUE),
            Xe2 = sum(Xe[Pair == "Site_2"], na.rm = TRUE),
            Xe3 = sum(Xe[Pair == "Site_3"], na.rm = TRUE),
...
yearly_effect_multiple_no_watershed <- yearly_multiple_control_no_watershed %>% 
  group_by(Study_ID, response_var, Climate, year) %>%
  mutate(lnR1 = log(Xe1/Xc),
         lnR2 = log(Xe2/Xc2),
         lnR3 = log(Xe3/Xc345),
...
# Adding columns to match brie_test_stats
yearly_effect_size_no_watershed <- yearly_effect_size_no_watershed %>% 
  add_column(Xc345 = NA,
             Xc2 = NA) %>% 
  ungroup()

# combining Hauer and Hickenbottom back into the mix 
yearly_effect_size_combine_no_watershed <- yearly_effect_size_no_watershed %>% 
  add_row(yearly_effect_multiple_no_watershed)

# Adding in more broad climate 
unique(yearly_effect_size_combine_no_watershed$Climate)

yearly_effect_size_combine_no_watershed <- yearly_effect_size_combine_no_watershed %>% 
  mutate(yearly_effect_size_combine_no_watershed, Biome = ifelse(grepl("Dfb|Dfc|Dsb", Climate), "Cold",
                              ifelse(grepl("Cfb|Csa|Csb|Cfa", Climate), "Temperate",
                              ifelse(grepl("BSk", Climate), "Arid",
                              ifelse(grepl("As|Af|Am", Climate), "Tropical",
                              ifelse(grepl("ET|EF", Climate), "Polar", "Other"))))))
...
# pivoting to make the responses in one column
# effect size 
yearly_effect_size_combine_long_no_watershed <- yearly_effect_size_combine_no_watershed %>% 
  pivot_longer(
    cols = lnR1:lnR5,
    names_to = "Effect_size_pair",
    values_to = "Effect_size_no_watershed",
    values_drop_na = TRUE
  ) %>% 
  filter(is.finite(Effect_size_no_watershed))

```

# 10/1/2025 - Add in Crandall/Abbott/Rhea
```{r - combining in studies that were formatted differently so they needed their own script}
# combine the yearly_effect_size_combine_long with Crandall, Abbott, and Rhea. 

Crandall_effect_size_no_watershed <- read_csv(here("Output_for_analysis", "01_Crandall_effect_size", "Crandall_effect_size_no_watershed.csv"),
                                na = c('-9999', 'N/A')) 

abbott_effect_size_no_watershed <- read_csv(here("Output_for_analysis", "02_Abbott_effect_size", "Abbott_effect_size_no_watershed.csv"),
                               na = c('-9999', 'N/A'))

rhea_effect_size_no_watershed <- read_csv(here("Output_for_analysis", "03_Rhea_effect_size", "Rhea_effect_size_no_watershed.csv"),
                             na = c('-9999', 'N/A'))


# Adding columns to yearly_effect_size_combine_long to match Crandall_effect_size and above_five_effect_size 
yearly_effect_size_combine_long_no_watershed <-yearly_effect_size_combine_long_no_watershed %>% 
  add_column(Xc34 = NA) %>% 
  ungroup() %>% 
  dplyr::select(Study_ID, response_var, Climate, year, Xe1, Xe2, Xe3, Xe4, Xe5, Xc, Xc2, Xc34, Xc345, Biome, Effect_size_pair, Effect_size_no_watershed)

# Adding columns to Crandall_effect_size to match yearly_effect_size_combine_long and above_five_effect_size 
Crandall_effect_size_no_watershed <-Crandall_effect_size_no_watershed %>% 
  add_column(Time_Since_Fire = as.numeric(0),
             Biome = "Temperate",
             Xe5 = NA,
             Xc345 = NA) %>% 
...
# Adding columns to abbott_effect_size to match yearly_effect_size_combine_long and Crandall_effect_size 
abbott_effect_size_no_watershed <-abbott_effect_size_no_watershed %>% 
  add_column(Xe2 = NA, 
             Xe3 = NA, 
             Xe4 = NA, 
             Xe5 = NA, 
...
# Adding columns to rhea_effect_size to match abbott_effect_size, yearly_effect_size_combine_long and Crandall_effect_size 
rhea_effect_size_no_watershed <-rhea_effect_size_no_watershed %>% 
  add_column(Xe4 = NA, 
             Xe5 = NA, 
             Xc2 = NA, 
             Xc34 = NA,
...
# combining all of them back into the mix back into the mix 
yearly_effect_size_combine_long_no_watershed <- yearly_effect_size_combine_long_no_watershed %>% 
  add_row(Crandall_effect_size_no_watershed) %>% 
  add_row(abbott_effect_size_no_watershed) %>% 
  add_row(rhea_effect_size_no_watershed)
...
# Joining the TSF and the yearly effect size combine long dataframe 
yearly_effect_size_combine_long_no_watershed <- left_join(yearly_effect_size_combine_long_no_watershed, TSF_year, by = c("Study_ID", "year")) %>% 
  distinct(Effect_size_no_watershed, .keep_all = TRUE) 

yearly_effect_size_combine_long_no_watershed$Time_Since_Fire <- as.character(yearly_effect_size_combine_long_no_watershed$Time_Since_Fire) # making time since fire a character 
...
```{r - clean up effect size if there are any mistakes csv}
manual_effect_size_no_watershed <- yearly_effect_size_combine_long_no_watershed 

manual_effect_size_no_watershed <- manual_effect_size_no_watershed %>% 
  mutate(Time_Since_Fire = case_when(Study_ID == 'Mast & Clow, 2008' & 
                                     year == '2003' ~ "0",
                                     TRUE ~ Time_Since_Fire))
...
yearly_effect_size_combine_long_no_watershed <- manual_effect_size_no_watershed

yearly_effect_size_combine_long_out_no_watershed <- yearly_effect_size_combine_long_no_watershed %>%
  mutate(across(where(is.character), ~ifelse(is.na(.), "N/A", .))) %>%
  mutate(across(where(is.numeric), ~ifelse(is.na(.), -9999, .)))


write_csv(yearly_effect_size_combine_long_out_no_watershed, here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_Size_40days_no_watershed.csv"))

unique(yearly_effect_size_combine_long_out_no_watershed$Study_ID)

```

### 10/1/2025 - EFFECT SIZE COMP BETWEEN NORMALIZATION AND NOT ###
```{r}
ES_no_watershed <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "archive", "Effect_Size_40days_no_watershed.csv")) %>% 
  dplyr::select(Study_ID, year, response_var, Effect_size_pair, Effect_size_no_watershed)

ES_orig <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "archive", "Effect_Size_40days.csv")) %>% 
  dplyr::select(Study_ID, year, response_var, Effect_size_pair, Effect_size)


# 1. Merge using `inner_join` by Study_ID, response_var, and Effect_size_pair
merged_data <- ES_orig %>%
  mutate(Effect_size_no_watershed = ES_no_watershed$Effect_size_no_watershed)

# 2. Filter for Relevant Columns
comparison_data <- merged_data %>% 
  dplyr::select(response_var, Effect_size_no_watershed, Effect_size)


# 3. Perform a Regression Analysis
regression_results <- comparison_data %>%
  group_by(response_var) %>%
  summarise(
    regression = list(lm(Effect_size_no_watershed ~ Effect_size, data = cur_data())),
    .groups = "drop"
  )

# 4. Extract and display regression summaries for each response_var
regression_summaries <- regression_results %>%
  mutate(summary = map(regression, summary))

# Print summaries for each response_var
regression_summaries %>%
  select(response_var, summary) %>%
  walk(~ {
    cat("\nResponse Variable:", .x$response_var, "\n")
    print(.x$summary)
  })

# 5. Plot
ggplot(merged_data, aes(x = Effect_size, y = Effect_size_no_watershed)) +
  geom_point(color = "blue", alpha = 0.6) + # Scatter points
  geom_smooth(method = "lm", color = "red") + # Regression line
  facet_wrap(~ response_var, scales = "free") + # Separate panels by response_var
  labs(
    title = "Regression Analysis by Response Variable",
    x = "Effect Size (Original)",
    y = "Effect Size (No Watershed)"
  ) +
  theme_minimal() +
...
ES_no_watershed <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_Size_40days_no_watershed.csv")) %>% select(Study_ID, response_var, Climate, Biome, Effect_size_no_watershed, Effect_size_pair, Time_Since_Fire)

ES_orig <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_Size_40days.csv")) %>% select(Study_ID, response_var, Climate, Biome, Effect_size, Effect_size_pair, Time_Since_Fire)
...
# Output the merged data frame for analysis:
write_csv(merged_data, here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_size_orig_nonnormal.csv"))
...
# ================================= Compare Thresholds ============================
```{r}
es_10 <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_Size_10days.csv")) %>% 
  select(Study_ID, response_var, Effect_size_pair, Effect_size) %>% 
  rename(Effect_size_10 = Effect_size)

es_20 <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_Size_20days.csv")) %>% 
  select(Study_ID, response_var, Effect_size_pair, Effect_size) %>% 
  rename(Effect_size_20 = Effect_size)

es_30 <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_Size_30days.csv")) %>% 
  select(Study_ID, response_var, Effect_size_pair, Effect_size) %>% 
  rename(Effect_size_30 = Effect_size)

es_40 <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_Size_40days.csv")) %>% 
  select(Study_ID, response_var, Effect_size_pair, Effect_size) %>% 
  rename(Effect_size_40 = Effect_size)

es_50 <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_Size_UPDATED.csv")) %>% 
  select(Study_ID, response_var, Effect_size_pair, Effect_size) %>% 
  rename(Effect_size_50 = Effect_size)


# TAKE 1 #
library(ggtukey)
 
merged_df <- bind_rows(es_10, es_20, es_30, es_40, es_50)

merged_df_long <- merged_df %>% 
  pivot_longer(
    cols = Effect_size_10:Effect_size_50,
    names_to = "Gaps",
    values_to = "Effect_Size",
    values_drop_na = TRUE,
  )
...
# Plot 
ggplot(merged_df_long, aes(x = Gaps, y = Effect_Size)) +
  geom_boxplot() +
  geom_jitter(alpha = 0.05) +
  scale_x_discrete(labels = c("Effect_size_10" = "10", "Effect_size_20" = "20", "Effect_size_30" = "30", "Effect_size_40" = "40", "Effect_size_50" = "50")) +
...
ggsave("interp_threshold_comp.pdf",
       here("initial_plots", "04_Meta_merge_all_studies_effect_size"),
       width = 10, height = 8, units = "in")


# Summary stats to make sure merge worked:
summary_10 <- es_10 %>% 
  group_by(response_var) %>% 
  summarise(mean = mean(Effect_size_10))

print(summary_10)

# Compare to the grouped one
summary_all <- merged_df_long %>% 
  group_by(response_var, Gaps) %>% 
...
# ================================= PLOTTING ==========================
```{r}
old_es <- read_csv(here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "Effect_Size.csv"))


# Merge the data frames 
merged_effect_size <- left_join(old_es, yearly_effect_size_combine_long_out, 
        by = c("Study_ID", "response_var", "year", "Effect_size_pair"),
        suffix = c("_old", "_new"))


# Perform a linear regression
regression_result <- lm(Effect_size_old ~ Effect_size_new, data = merged_effect_size)

# Create the plot with equation and R² value
regression_plot <- ggplot(merged_effect_size, aes(x = Effect_size_old, y = Effect_size_new)) +
  xlim(-3,3) +
...
count_climate <- under_ten_long %>% 
  count(Study_ID)
sum(count_climate$n) # Should be 49
# 49 watersheds are included in the climate analysis 
...
#          day = day(Sampling_Date))
#   
# write_csv(control_impact_fill, here("Output_for_analysis", "04_Meta_merge_all_studies_effect_size", "gap_fill.csv"))
